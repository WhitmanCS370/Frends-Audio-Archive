...
class SliderInput:
    def __init__(self, name, current, type, min, max):
        ...
        self.slider = Slider(
            min=min, max=max, value=current, size_hint=(1, 2), height=40
        )
        self.name_label = Label(text=self.name)
        ...
        self.value_label = Label(
            text=str(round(self.type(current), 2)),
            size_hint=(None, None),
            size=(100, 40),
    ...
    def update_value(self, instance, value):
        ...
        self.value_label.pos_hint = {
            "center_x": ((instance.value - self.min) / (self.max - self.min))
        }
...
class DoubleRangeSliderInput:
    ...
    def update_range_label(self):
        self.range_label.text = (
            f"{int(self.slider_min.value)}-{int(self.slider_max.value)}%"
        )
