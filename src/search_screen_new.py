...
class SearchScreenLayout(BoxLayout):
    ...
    def search_sound(self, instance):
        ...
        def submit_results(instance): #also functions to pass the token back to the GUI manager!
...
class SearchResults(BoxLayout):
    def __init__(self, results, popup_instance, **kwargs):
        ...
        for result in results:
            #print(result)
            box = MetadataDisplay(result, name=result.name, on_checkbox_active=self.checkbox_active)
            self.add_widget(box)
...
class MetadataDisplay(BoxLayout):
    def __init__(self, dataList, name, on_checkbox_active, **kwargs):
        ...
        self.checkbox.bind(active=lambda instance, value: on_checkbox_active(instance, value, self.name))
